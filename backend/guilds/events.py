...
class GuildEvents(commands.Cog):
    ...
    async def on_guild_join(self, guild):
        ...
        create_or_update_guild(self.bot, guild.id, added_at=get_utc_now(), is_active=True)

    @commands.Cog.listener()
    async def on_guild_remove(self, guild):
        print(f"A guild named {guild.name} left! #{guild.id}")
        create_or_update_guild(self.bot, guild.id, removed_at=get_utc_now(), is_active=False)
